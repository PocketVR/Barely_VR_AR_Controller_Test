...
// Begin JS library code
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(8, 0, 1, target, width, height);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(9, 0, 1, index, gamepadState);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(24, 0, 1, title);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(25, 0, 1, fd);
...
var ASM_CONSTS = {
  3560712: () => { return window.screen.width ; },  
 3560745: () => { return window.screen.height; },  
 3560778: ($0, $1) => { var data = JSON.stringify( window.sensor ); stringToUTF8( data, $0, $1 ); return data.length; },  
 3560876: () => { return window.screen.width; },  
 3560908: () => { return window.screen.height; },  
 3560941: () => { (()=>{ window.addEventListener( "resize", ()=>{ engine.refresh(); }); window.sensor = new Object({ x:"0", y:"0", z:"0", w:"0" }); try { const sensor = new AbsoluteOrientationSensor({ frequency: 60 }); sensor.addEventListener( "reading", ()=>{ window.sensor.x = sensor.quaternion[0].toFixed(6); window.sensor.y = sensor.quaternion[1].toFixed(6); window.sensor.z = sensor.quaternion[2].toFixed(6); window.sensor.w = sensor.quaternion[3].toFixed(6); }); sensor.start(); } catch(err) { alert( "gyroscope not found" ); } })(); },  
 3561467: ($0, $1) => { let data = Date.now() + ""; stringToUTF8( data, $0, $1 ); return data.length; },  
 3561549: () => { if (document.fullscreenElement) return 1; },  
 3561595: () => { return document.getElementById('canvas').width; },  
 3561647: () => { return parseInt(document.getElementById('canvas').style.width); },  
 3561715: () => { document.exitFullscreen(); },  
 3561742: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 3561815: () => { if (document.fullscreenElement) return 1; },  
 3561861: () => { return document.getElementById('canvas').width; },  
 3561913: () => { return screen.width; },  
 3561938: () => { document.exitFullscreen(); },  
 3561965: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 3562098: () => { return window.innerWidth; },  
 3562124: () => { return window.innerHeight; },  
 3562151: () => { if (document.fullscreenElement) return 1; },  
 3562197: () => { return document.getElementById('canvas').width; },  
 3562249: () => { return parseInt(document.getElementById('canvas').style.width); },  
 3562317: () => { if (document.fullscreenElement) return 1; },  
 3562363: () => { return document.getElementById('canvas').width; },  
 3562415: () => { return screen.width; },  
 3562440: () => { return window.innerWidth; },  
 3562466: () => { return window.innerHeight; },  
 3562493: () => { if (document.fullscreenElement) return 1; },  
 3562539: () => { return document.getElementById('canvas').width; },  
 3562591: () => { return screen.width; },  
 3562616: () => { document.exitFullscreen(); },  
 3562643: () => { if (document.fullscreenElement) return 1; },  
 3562689: () => { return document.getElementById('canvas').width; },  
 3562741: () => { return parseInt(document.getElementById('canvas').style.width); },  
 3562809: () => { document.exitFullscreen(); },  
 3562836: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 3562894: () => { return screen.width; },  
 3562919: () => { return screen.height; },  
 3562945: () => { return window.screenX; },  
 3562972: () => { return window.screenY; },  
 3562999: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 3563052: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 3563123: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 3563180: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 3563436: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 3563507: () => { if (document.fullscreenElement) return 1; },  
 3563553: () => { return window.innerWidth; },  
 3563579: () => { return window.innerHeight; },  
 3563606: () => { if (document.pointerLockElement) return 1; }
};
